...
for i, label in enumerate(labels):
    ax.plot(ψ_history[:, i], label=label, lw=2)

mc_h = qe.MarkovChain(P_hamilton)
ψ_star_h = mc_h.stationary_distributions[0]
for i in range(3):
    ax.axhline(ψ_star_h[i], color='k', linestyle='--', α=0.3)
